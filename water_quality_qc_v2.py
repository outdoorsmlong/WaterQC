...
# Bump this whenever you change the engine. The Streamlit app displays it
# at the top of the page so you can confirm the deployed version matches
# what you pushed. If you fix a bug but still see the old behavior, check
# that the deployed __ENGINE_VERSION__ matches the latest in this file.
__ENGINE_VERSION__ = "2.4.0"  # 2026-06-25: defensive numeric coercion in every detector
...
class WaterQualityQCv2:
    ...
    def _ensure_numeric(series: pd.Series) -> pd.Series:
        """Idempotent numeric coercion — safe to call on any input.

        Defense in depth: even if a caller forgot to coerce, every detector
        below will receive a numeric series. Handles object/string/PyArrow
        string dtypes that pandas 2.x refuses to compare to numbers.
        """
    ...
    def _flag_range(self, series: pd.Series, cfg: ParameterConfig) -> pd.Series:
        series = self._ensure_numeric(series)
